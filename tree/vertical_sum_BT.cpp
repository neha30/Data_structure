//program to print vertical sum of binary tree
...
void print_node(struct node *root,int val,int hd,int min,int *sum)
{
	//base case
	if(root==NULL)
		return;
	...
	{
		cout<<root->data<<" ";
		sum[val-min]+=root->data;
	}
		
	
	
	//otherwise call function recur
	print_node(root->left,val,hd-1,min,sum);
	print_node(root->right,val,hd+1,min,sum);
}
...
void vertical_sum(struct node *root)
{
   ...
   for(i=min;i<=max;i++)
   {
   	   print_node(root,i,0,min,sum);
   	   cout<<endl;
   }

   cout<<"sum of vertical line:\n";
